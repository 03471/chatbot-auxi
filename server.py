...
# prepare data
from data.ubuntu import data

# Just disables the warning, doesn't enable AVX/FMA
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'

metadata, idx_q, idx_a = data.load_data(PATH='data/ubuntu/')
...
def answer(message):
    seed_id = [w2idx[w] for w in message.split(" ")]
    ...
    for _ in range(10):  # max sentence length
        ...
        #     break
        sentence = sentence + [w]
    m = {'message': ' '.join(sentence), 'author': 'auxi', 'createDate': time.strftime('%H:%M:%S')}
    emit('message', {'data': m}, broadcast=True)
